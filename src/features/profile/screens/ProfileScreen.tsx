...
import { useCreditCardSummary } from "@/features/creditCards/services/creditCardsApi";
import {
  useMyProfile,
...
function SettingsRow({
  ...
  return (
    <Pressable
      ...
      style={({ pressed }) => [
        sRowStyles.row,
        pressed && onPress && sRowStyles.rowPressed,
      ]}
    >
      <Ionicons
        name={icon}
...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    if (profile) {
      if (profile.monthlyBudgetCLP != null)
        setBudgetCLP(String(profile.monthlyBudgetCLP));
      if (profile.monthlyBudgetUSD != null)
        setBudgetUSD(String(profile.monthlyBudgetUSD));
    }
  }, [profile]);
  ...
  const handleSaveBudget = async () => {
    try {
      ...
      if (!isSessionExpired())
        Alert.alert(
          "Error",
          e instanceof Error ? e.message : "No se pudieron guardar",
        );
    }
  };
  ...
  return (
    ...
    >
      ...
      <View style={s.profileHeader}>
        ...
        <View style={s.badge}>
          <Ionicons name="logo-google" size={13} color={colors.accent} />
          <Text style={s.badgeText}>Google</Text>
        </View>
      </View>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Pressable
              ...
              style={({ pressed }) => [
                s.saveButton,
                updateMyProfileMutation.isPending && { opacity: 0.6 },
      ...
      <View style={styles.card}>
        <SettingsRow
          ...
          value=""
          showChevron
          onPress={() => router.push("/(tabs)/perfil/notificationSettings")}
        />
        <SettingsRow
      ...
      <Pressable
        ...
        style={({ pressed }) => [s.logoutCard, pressed && { opacity: 0.7 }]}
      >
